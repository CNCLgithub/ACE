...
function test_wm()
    istate = WorldState([
        Disc(S2V(-50, 0), S2V(1, 0), 10.0),
        Disc(S2V(50, 0), S2V(-1, 0), 10.0),
        Disc(S2V(0, 0), S2V(0.5, -1), 10.0),
    ])
    ...
    new_tr, ls, _... = regenerate(tr, select(:states => time => :forces))
    @show ls
	...
	for t = 1:time
		# Visualizations
         snapshots[t] = hcat(
            paint_state(states[t], wm, true), # gt state
            # Receptive Fields colored by Mean RGB
            paint_state(wm.graphics, states[t]; mode=:mean, show_objects=false, back_color="black"),
            # Receptive Fields colored by Variance
            paint_state(wm.graphics, states[t]; mode=:variance, show_objects=false, back_color="black"),
            hpad=10
         )
